...
def getGlobals():
    ...
    global R_NUC, Z_NUC
    R_NUC = np.zeros( (2,3) ) # One H, One Li, 3 dimensions
    R_NUC[0,0] = 0.0 # Li
    R_NUC[0,0] = 1.65 # H
    Z_NUC = np.zeros( (2) )  # One H, One Li
    Z_NUC[0] = 3  # One Li
    Z_NUC[1] = 1  # One H

    global dimension, particles, interacting
    dimension   = 3 # Can do many dimensions
    particles   = 4 # Don't do more than ~3 particles. Need to optimize code first.
    interacting = True

    global E_TRIAL_0
    E_TRIAL_0 = -8.0 # Choose best guess
