...
def DMC( PARAM, positions=None ):
    ...
    if ( positions is None ):
        ...
        #         print(NSTART, NEND, num_walkers)
        #         positions[:,NSTART:NEND,dim]  = (np.random.uniform(size=(particles,NEND-NSTART))*2-1)*10
        
        for dim in range(dimension):
            positions[:,:,dim]  = (np.random.uniform(size=(particles,num_walkers))*2-1)*10
    ...
    for step in range(num_steps):
        ...
        energy_traj[step,0] = np.average( potential_energies_old )
        print( step, potential_energies_old[-1], energy_traj[step,0] )
        exit()
        energy_traj[step,1] = np.var( potential_energies_old )
        energy_traj[step,2] = np.std( potential_energies_old )
