...
def get_potential(PARAM, x=None):
    if ( x is None ):
        x = PARAM["POSITIONS"]
    ...
    if ( len(x.shape) == 3 ):
        NELECTRONS, NDIMENSIONS  = x.shape
        V_EL = np.zeros( (NELECTRONS) )
        ...
        if ( interacting == True ):
            for p1 in range( NELECTRONS ):
                for p2 in range( p1+1, NELECTRONS):
                    if ( p1 != p2 ):
                        Ree = np.einsum( "d->", (x[p1]-x[p2])**2 ) ** (1/2)
                        V_EL[p1] += 1 / Ree
        ...
        for p1 in range(NELECTRONS): # Loop over particles
            for Ri, R in enumerate( R_NUC ): # Loop over nuclei
                ...
                ReN = np.einsum( "d->", ReN**2 ) ** (1/2)
                V_EL[p1] -= Z_NUC[Ri] / ReN
        
        # Add all QM particles
        V_EL = np.einsum("p->", V_EL[:]) # Note: Shape Change
        ...
        if ( PARAM["DO_POLARITON"] == True ):
            ...
            MU_EL       = np.einsum("ed->d", x[:,:] ) # Sum over electrons
            MU_TOT      = np.einsum("d,d->d", MU_NUC[:], -1.0000 * MU_EL[:]) # (w,d)
            MU_TOT_PROJ = np.einsum( "d,d->", MU_TOT, EPOL ) # Project along field polarization
            ...
            for p in range( NELECTRONS ):
                for pp in range( NELECTRONS ):
                    ...
                    T1 += np.einsum( "d,d->", x[p,:] * x[pp,:], EPOL )
            ...
            for p in range( NELECTRONS ):
                for Ri, R in enumerate( R_NUC ):
                    ...
                    T2 += -2.000 * np.einsum( "d,d->", x[p,:] * Z_NUC[Ri] * R_NUC[Ri,:], EPOL )
