...
def getGlobals():
    ...
    global R_NUC, Z_NUC
    R_NUC = np.zeros( (1,3) ) # One Helium, 3 dimensions
    R_NUC[0,0] = 0.0 # He
    Z_NUC = np.zeros( (2) )  # One He
    Z_NUC[0] = 2  # One He

    global dimension, particles, interacting
    dimension   = 3 # Can do many dimensions
    particles   = 2 # Don't do more than 3 particles. Need to optimize code first.
    interacting = True
...
def DMC():
    ...
    for Ri,R in enumerate( R_NUC ):
        ...
        positions[:,NSTART:NEND,:]  = np.random.normal(R, 2, size=(particles,NEND-NSTART,dimension))
